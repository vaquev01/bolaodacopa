...
- ✅ **v1.5 — "Só classificação" centrado no chaveamento**: bracket pré-ligado (1º/2º por grupo + 8 melhores 3ºs + fases até o campeão), toggle "Cravar placar vale pontos extras" (variant `specials_plus`: aba de jogos visível, só placar em cheio pontua — ruleset com demais camadas 0), e **`BracketBoard`** — chaveamento visual completo estilo tabela de jornal na tab Bracket (12 grupos + melhores 3ºs + árvore do mata-mata, palpite × resultado real: acerto verde, erro riscado). Smoke e2e: bracket e placar extra aceitos pelo servidor com a Copa em andamento
