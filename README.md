...
- ✅ Scoring Engine (`src/lib/scoring/`) — TDD, 130 testes passando (ruleset Zod + engine puro + standings + bracket com melhores 3ºs + winner mode). Regra de consolação `goals_one_team` (errou vencedor mas acertou gols de um time; default 1pt, 0 desliga) e defaults rebalanceados: exato 10 / vencedor+saldo 7 / vencedor 4 / empate 4 / consolação 1
